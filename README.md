...
（既定の 60 では OBS のページまで退避される）。配信開始の直前にメモリを
...
| `Committed_AS` ÷ (RAM+swap) | `OBS_MAX_COMMIT_RATIO` 100% | 「全員が確保したぶんを触ったら swap に落ちる」状態。**2026-09-09 を拾えるのはこれだけ** |

2026-09-09 の配信がこの点検を素通りした。`MemAvailable` は 15.3GB あったが、
**これは回収できるページキャッシュを含む**ので、匿名ページで RAM が埋まって
いても大きく出る。その裏で swap は 6.2GB 使われ commit は 110% だった。
