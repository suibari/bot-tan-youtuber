...
def clear_comments() -> None:
    """コメント欄を空にする。配信の始めと終わりに呼ぶ。

    comments.txt はファイルに残り続けるので、消さないと**配信開始から最初の
    コメントが来るまで前回配信のコメントが出たまま**になる（初コメで上書きされて
    消えるので、視聴者からは「始まってすぐ古いコメントが並んでいる」に見える）。
    字幕と違って websocket 経路には載せていない（OBS がファイルを読むのは
    最大1秒後だが、コメント欄は発話と同期しないので問題にならない）。
    """
