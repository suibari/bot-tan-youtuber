...
#
# 原因は Unity でも配信スクリプトでもなく、:99 の Xorg の表示経路が
# 停止していたこと。glxgears が 1.0〜1.4fps しか出ず、GPU 使用率は 0%、
# GPU の計算そのものは正常だった。この状態の Unity は PresentContextGL で
# abort する（同じスタックで2回）。
#
# 当時は :0 も 1〜2fps だったことから「ホスト全体の故障」と読んだが、これは
# 誤りだった。ロックされた X11 セッションの GL は正常時も進まない（2026-09-11
# の実測では :99 が 60fps のときに :0 は1フレームも進まなかった）。
#
# 本当の原因は、GPU が1枚しかないのに X サーバが2つあること。:99 は gdm より
# 後に起動していないと描画を締め出され、GPU が空いていても 1.0fps に張り付く。
# 起動順は保証できないので、配信の直前に作り直す（setup/reset_display.sh を
# bottan-live-display.service が呼ぶ）。
