...
#
# 追記（2026-09-19）: この移設が 09/09 の真因への対処そのものだった。移設後の
# 09/17・09/18 は swap 48〜50% のまま skipped_frames 0 で完走している。当初
# ここには「根本の commit 110% は別途 RAM を空けて対処すること」と書いていたが、
# commit 比は落ちた日のほうが低く（09/09 110% に対し完走した 09/17〜18 は
# 130% 超）、指標として判別力が無かった。詳細は README のメモリの節を参照。
# スワップが使われること自体は害ではない。見るべきは詰まり（PSI io）のほう。
#
